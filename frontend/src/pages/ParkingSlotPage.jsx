...
const ParkingSlotPage = () => {
  ...
  const filteredSlots = slots.filter((slot) => {
    // Filter by location
    if (filters.location && !slot.location.toLowerCase().includes(filters.location.toLowerCase())) {
      return false;
    }
    ...
    if (filters.availability === 'available' && !slot.is_active) {
      return false;
    }
    if (filters.availability === 'occupied' && slot.is_active) {
      return false;
    }
  ...
  return (
    <>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-4">
          <p className="text-lg">
            {filteredSlots.length} slots found • {filteredSlots.filter(s => s.is_active).length} available
          </p>
        </div>

        {Object.keys(slotsByLocation).length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-500">No parking slots matching your criteria</p>
          </div>
        ) : (
          Object.entries(slotsByLocation).map(([location, slots]) => (
            <div key={location} className="mb-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {slots.map((slot) => (
                  <Card key={slot.id} className={`${slot.is_active ? 'border-green-500 border-2' : 'border-red-300 border'}`}>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      >
                        {slot.is_active ? 'Available' : 'Occupied'}
                      </span>
                    </div>
                    ...
                    <p className="mb-3">
                      <span className="font-medium">Floor:</span> {slot.floor || 'Ground level'}
                    </p>

                    {slot.is_active && (
                      <Link to="/booking" state={{ preselectedLocation: location, preselectedType: slot.type }}>
