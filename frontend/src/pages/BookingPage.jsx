...
const BookingPage = () => {
  ...
  useEffect(() => {
    ...
    const endTime = formatDateTime(new Date(now.getTime() + 2 * 60 * 60 * 1000));
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      setIsLoading(true);
      try {
        const response = await api.post('/bookings/create-by-location', formData);
        ...
        setErrorMessage(
          error.response?.data?.message || 'An error occurred while creating your booking'
        );
      } finally {
        setIsLoading(false);
      }
            ...
            <Card title="Booking Successful!">
              <div className="space-y-4">
                <div>
                  ...
                  <p>Status: <span className="capitalize">{bookingData.status}</span></p>
                  <p>Location: {bookingData.parking_slot?.location || 'Unknown location'}</p>
                  <p>Slot: {bookingData.slot_id}</p>
                  <p>Start: {new Date(bookingData.start_time).toLocaleString()}</p>
                  <p>End: {new Date(bookingData.end_time).toLocaleString()}</p>
                  <p className="font-medium mt-2">Price: ${bookingData.price}</p>
                </div>
